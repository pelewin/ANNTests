package com.pelevin.gameengine;
...
public class SpriteManager {
	 ...
	 */
	private final static Map<Integer, List<Sprite>> GAME_ACTORS = new HashMap<>();
	...
	/** */
	public Map<Integer, List<Sprite>> getAllSprites() {
		return GAME_ACTORS;
	}

	public List<Sprite> getSpritesByType(int type){
		List<Sprite> actorsList = GAME_ACTORS.get(type);
		if (actorsList == null) {
	 ...
	 */
	public void addSprites(int type, Sprite... sprites) {
		List<Sprite> actorsList = GAME_ACTORS.get(type);
		if (actorsList == null) {
			actorsList = new ArrayList<>();
			GAME_ACTORS.put(type, actorsList);
		}
		actorsList.addAll(Arrays.asList(sprites));
	}

	/**
	 * VarArgs of sprite objects to be removed from the game.
	 *
	 * @param sprites Sprites to remove
	 */
	public void removeSprites(int type, Sprite... sprites) {
		List<Sprite> actorsList = GAME_ACTORS.get(type);
		if (actorsList != null) {
			actorsList.removeAll(Arrays.asList(sprites));
		}
	}
	...
	public void addSpritesToBeRemovedByType(int type){
		CLEAN_UP_SPRITES.addAll(getSpritesByType(type));
	}
	...
	public void resetCollisionsToCheck() {
		...
		GAME_ACTORS.values().forEach(CHECK_COLLISION_LIST::addAll);
	}
	...
	public void cleanupSprites() {
		...
		if (CLEAN_UP_SPRITES.size() == 0) return;

		// remove from actors list
		for (List<Sprite> sprites : GAME_ACTORS.values()) {
